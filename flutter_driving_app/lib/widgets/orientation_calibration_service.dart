...
class OrientationCalibrationService {
  ...
  void _transportBasisForLargeUpChange(List<double> currentUp) {
    ...
    final crossX = previousUp[1] * currentUp[2] - previousUp[2] * currentUp[1];
    final crossY = previousUp[2] * currentUp[0] - previousUp[0] * currentUp[2];
    final crossZ = previousUp[0] * currentUp[1] - previousUp[1] * currentUp[0];
    final crossNorm = math.sqrt(
      crossX * crossX + crossY * crossY + crossZ * crossZ,
    );
    if (crossNorm < 1e-9) return;

    final angle = math.atan2(crossNorm, dotUp);
    final rotatedForward = _rotateVectorAroundAxis(forwardAxis, [
      crossX / crossNorm,
      crossY / crossNorm,
      crossZ / crossNorm,
    ], angle);
  ...
  void _transportBasisForRelativeYaw(
    ...
    final forwardAxis = _forwardAxis;
    if (forwardAxis == null ||
        relativeBasisYawRadians.abs() < _basisYawTransportThresholdRadians) {
      return;
    }
  ...
  void _updateForwardAxis(
    ...
    if (existingDotCandidate < _forwardAxisAgreementCosine) {
      ...
      if (pending == null) {
        _pendingLargeRemountForwardAxis = [candidateX, candidateY, candidateZ];
